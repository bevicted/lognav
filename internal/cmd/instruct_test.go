...
func TestInstructContentContracts(t *testing.T) {
	...
	assert.Contains(t, query, "known bounded ranges are\n   more efficient than broad relative ranges")
	assert.Contains(t, query, "| f $l.subsystemname == 'service'")
	assert.Contains(t, query, "$d.log")
	assert.Contains(t, query, "Never infer or invent a field name")
	assert.Contains(t, query, "$d ~~ 'text'")
	assert.Contains(t, query, "field is grounded in the user's query")
	assert.Contains(t, query, "field ~ 'text'")
	assert.Contains(t, query, "distinctive dash-free segment")
	assert.Contains(t, query, "workaround to labels; preserve the full label value")
	assert.NotRegexp(t, `\$d[[:space:]]*~~[[:space:]]*'[^']*-[^']*'`, query)
	assert.Equal(t, 1, strings.Count(queryProcedure, "Never infer or invent a field name"))
